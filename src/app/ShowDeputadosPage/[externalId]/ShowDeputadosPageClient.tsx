...
export default function ShowDeputadosPageClient({
  ...
  const productivityLabel =
    deputado.productivity_bills_per_year !== null
      ? Number(deputado.productivity_bills_per_year).toLocaleString('pt-BR', { maximumFractionDigits: 1 })
      : '—';

  const topTopic = deputado.thematic_focus_top_topic;
  const topTopicSharePct =
    deputado.thematic_focus_top_topic_share !== null
      ? `${Math.round(Number(deputado.thematic_focus_top_topic_share) * 100)}%`
      : null;
  const topicoIndiceExibicao =
    deputado.thematic_focus_index !== null ? `${Math.round(Number(deputado.thematic_focus_index) * 100)}%` : '—';
